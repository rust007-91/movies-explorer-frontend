...
function AuthWithForm(props) {
    ...
    const {title, name, btnText, questionText, linkText, route, onSubmit} = props;
    const {values, handleChange, errors, isInputValid, isFormValid, resetForm} = useFormWithValidation(); // хук обработки поле
                ...
                >
                    ...
                    <fieldset className="auth__form-fieldset">
                        {location.pathname === '/signup' && (
                            <>
                                <label htmlFor='name__auth' className="auth__label">Имя
                                    <input
                                        id='name__auth'
                                        type="text"
                                        className={`auth__input ${isInputValid ? "" : "auth__input_error"}`}
                                        name='name'
                                        minLength='5'
                                        maxLength='30'
                                        required
                        ...
                        <label htmlFor='email__auth' className="auth__label">E-mail
                            <input
                                id='email__auth'
                                type="email"
                                className={`auth__input ${isInputValid ? "" : "auth__input_error"}`}
                        ...
                        <label htmlFor='password__auth' className="auth__label">Пароль
                            <input
                                id='password__auth'
                                type="password"
                                minLength="2"
                    ...
                    <div className="auth__btn-wrap">
                        ...
                        <button
                            type='submit'
                            className={`auth__btn ${!isFormValid && "auth__btn_disabled"}`}
